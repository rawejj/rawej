...
import { saveToken, loadToken, clearToken, TokenData } from '@/utils/token-storage';

export function isTokenValid(token: TokenData | null): boolean {
  return !!token && typeof token.accessToken === 'string' && token.accessToken.length > 0 && token.expiresAt > Date.now();
}
...
export async function fetchToken(): Promise<string> {
  ...
  try {
    const tokenResp = await httpClient<{ accessToken: string; refreshToken: string; expiresIn?: number }>(tokenUrl, {
      method: 'POST',
...
export async function refreshAccessToken(): Promise<string> {
  ...
  try {
    const resp = await httpClient<{ accessToken: string; refreshToken?: string; expiresIn?: number }>(refreshUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
