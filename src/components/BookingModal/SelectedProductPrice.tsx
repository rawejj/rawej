  ...
  return (
    <div className="mb-4 px-4 py-3 rounded-xl border border-purple-200 dark:border-pink-700 bg-linear-to-r from-purple-50 to-pink-50 dark:from-pink-950/30 dark:to-purple-900/30 flex flex-col gap-1 shadow-sm">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm flex-1 font-medium text-purple-600 dark:text-pink-300">{price.title}</span>
        {price.discount_amount > 0 ? (
          <>
            <span className="text-sm font-bold text-gray-400 dark:text-gray-500 line-through">{price.price.toLocaleString()} {price.currency}</span>
            ...
            <span className="text-sm font-bold text-green-700 dark:text-green-300">{(price.price - price.discount_amount).toLocaleString()} {price.currency}</span>
          </>
        ) : (
          <span className="text-sm font-bold text-purple-900 dark:text-pink-200">{price.price.toLocaleString()} {price.currency}</span>
        )}
      </div>
    </div>
  );
