...
describe('Constants', () => {
  describe('SUPPORTED_LANGUAGES', () => {
    ...
    it('all languages are valid LanguageKey type', () => {
      const validLanguages: LanguageKey[] = ['en', 'de', 'fr', 'ku-sor', 'ku-kur', 'fa'];
  ...
  describe('Integration', () => {
    ...
    it('supports multiple languages with different scripts', () => {
      const hasLatin = SUPPORTED_LANGUAGES.some((lang) => ['en', 'de', 'fr'].includes(lang));
      const hasArabic = SUPPORTED_LANGUAGES.some((lang) => lang === 'fa');
      const hasKurdish = SUPPORTED_LANGUAGES.some((lang) => lang.startsWith('ku-'));
